    ...
    c.on('packet', () => {
      ...
      if (ret.info.protocol === decoders.PROTOCOL.IP.UDP) {
        ...
        try {
          ...
          Stats.counters.packetsOk++
        } catch (error) {
          // Paquets tronqués/mal alignés arrivent en continu en capture passive (frames
          // perdues, trafic non-Photon sur le même port...) — normal, pas une vraie
          // erreur : uniquement tracé dans debug-logs.txt (voir utils/logger.js).
          Stats.counters.packetsFailed++
          Logger.debug('error parsing photon packet', error)
          Logger.debug('packet', prettyPrintBuffer(packet))
        }
      }
    })
